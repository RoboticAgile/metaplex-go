...
var FreezeStateDiscriminator = [8]byte{86, 61, 245, 200, 4, 134, 233, 121}

func (obj FreezeState) MarshalWithEncoder(encoder *ag_binary.Encoder) (err error) {
	// Write account discriminator:
	err = encoder.WriteBytes(FreezeStateDiscriminator[:], false)
	if err != nil {
		return err
	}
...
func (obj *FreezeState) UnmarshalWithDecoder(decoder *ag_binary.Decoder) (err error) {
	// Read and check account discriminator:
	{
		discriminator, err := decoder.ReadTypeID()
		if err != nil {
		...
		if !discriminator.Equal(FreezeStateDiscriminator[:]) {
			return fmt.Errorf(
				"wrong discriminator: wanted %s, got %s",
				"[86 61 245 200 4 134 233 121]",
				fmt.Sprint(discriminator[:]))
		}
	}
	// Deserialize `Expiry`:
...
func (obj WalletLimitInfoPerStage) MarshalWithEncoder(encoder *ag_binary.Encoder) (err error) {
	// Write account discriminator:
	err = encoder.WriteBytes(WalletLimitInfoPerStageDiscriminator[:], false)
	if err != nil {
		return err
	}
	// Serialize `Redeemed` param:
	err = encoder.Encode(obj.Redeemed)
	if err != nil {
		return err
	}
	return nil
...
func (obj *WalletLimitInfoPerStage) UnmarshalWithDecoder(decoder *ag_binary.Decoder) (err error) {
	// Read and check account discriminator:
	{
		...
		if !discriminator.Equal(WalletLimitInfoPerStageDiscriminator[:]) {
			return fmt.Errorf(
				"wrong discriminator: wanted %s, got %s",
				"[109 39 175 252 165 11 210 251]",
				fmt.Sprint(discriminator[:]))
		}
	}
	// Deserialize `Redeemed`:
	err = decoder.Decode(&obj.Redeemed)
	if err != nil {
		return err
	}
	return nil
